...
A Windows host answers for itself, and not every one of them lets a journey
answer for another: the windows-11-arm runner's PowerShell reads `ARM64`
whatever processor it was started with, where the windows-2025 runner's reads
the one it was handed. So which Windows platforms a fixture can name here is
asked of this host's PowerShell once, and the tests that name one take it
from that answer — the host's own always among them.
...
def _here() -> str | None:
    """This host's own Windows platform, or nothing off Windows."""
    return platforms.host(REPO).id if sys.platform == "win32" else None


def _reads_what_it_is_handed(machine: str) -> bool:
    """Whether this host's PowerShell reads a processor it was started with.

    Asked of PowerShell itself: started with `machine` in the environment the
    way the script is, and asked for the two variables the script reads in
    the order it reads them. Off Windows nothing rewrites a process's
    environment, so the answer is yes without asking.
    """
    if sys.platform != "win32":
        return True
    environment = clean_environment(PROCESSOR_ARCHITECTURE=machine)
    ...
    result = shell_run([powershell(), "-NoProfile", "-Command", read], env=environment, timeout=120)
    return (result.stdout or "").strip() == machine


@functools.cache
def _stood_in() -> list[str]:
    """The Windows platforms a fixture can name on this host, this host's own first.

    Off Windows every one of them. On a Windows host its own, and each other
    whose processor its PowerShell reads when handed it — which the
    windows-2025 runner's does and the windows-11-arm runner's does not.
    """
    here = _here()
    if here is None:
        return list(WINDOWS)
    return [here] + [
        other for other in WINDOWS if other != here and _reads_what_it_is_handed(_machine(other))
    ]


def _first() -> str:
    """The platform the single-platform tests stand in: this host's own, where it is Windows."""
    return _stood_in()[0]
...
#: How the user's own PATH — the one Windows keeps for the next window — is read.
USERS_OWN_PATH = "[System.Environment]::GetEnvironmentVariable('Path', 'User')"


def _users_own_path() -> str:
    """The user's own PATH on this Windows host, read by a fresh PowerShell."""
    result = shell_run([powershell(), "-NoProfile", "-Command", USERS_OWN_PATH], timeout=120)
    return (result.stdout or "").strip()


@pytest.fixture(autouse=True)
def users_own_path_kept() -> Iterator[None]:
    """On a Windows host, the user's own PATH is put back after every test.

    The script persists the directory it installed into there, so every
    install this journey drives would otherwise leave one entry behind: on a
    runner a list of temporary directories, on a developer's machine one
    that grows with every run.
    """
    if sys.platform != "win32":
        yield
        return
    before = _users_own_path()
    yield
    kept = before.replace("'", "''")
    shell_run(
        [
            ...
            "-NoProfile",
            "-Command",
            f"[System.Environment]::SetEnvironmentVariable('Path', '{kept}', 'User')",
        ],
        timeout=120,
    )
...
def test_it_installs_the_newest_release_into_the_default_directory_and_says_what_is_next(
    ...
    now on the path, and the two commands the operator runs after it. Once per
    Windows platform this host can stand in — both, off Windows — and on a
    Windows host the program its own artifact carries is the real one.
    """
...
def test_a_pinned_release_is_the_one_installed(
    ...
    code, said = _script(staged(_first()), home, _first(), "-Version", OLDER)
...
def test_an_altered_artifact_is_refused_before_anything_reaches_a_path(
    ...
    into = home / "bin"
    base = staged(_first())
    altered = base / "latest" / "download" / platforms.descriptor(REPO, _first()).asset
    altered.write_bytes(altered.read_bytes() + b"an alteration nobody published")

    code, said = _script(base, home, _first(), "-To", str(into))
...
def test_a_processor_it_publishes_nothing_for_stops_with_a_next_action(
    ...
    if not _reads_what_it_is_handed("x86"):
        pytest.skip(
            "this host's PowerShell reads the machine's own processor over the one it is "
            "started with, so no fixture can name it one the release publishes nothing for; "
            "proven on every host whose PowerShell reads what it is handed"
        )
...
def test_a_host_that_is_not_windows_is_sent_to_the_shell_form(
    ...
    environment = _environment(staged(_first()), home, None)
...
def test_a_download_that_cannot_be_obtained_stops_with_a_next_action(
    ...
    code, said = _script(staged(_first()), home, _first(), "-Version", "v9.9.9")
...
def test_a_digest_file_naming_no_digest_for_the_artifact_is_refused(
    ...
    code, said = _script(base, home, _first())
...
def test_an_artifact_carrying_no_program_is_refused(
    ...
    asset = into / platforms.descriptor(REPO, _first()).asset
    raw = io.BytesIO()
    with tarfile.open(fileobj=raw, mode="w") as archive:
...
def test_the_piped_form_installs_and_leaves_the_session_with_a_zero_status(
    ...
    code, said = _piped(home, _environment(staged(_first()), home, _first()))
...
def test_the_piped_form_that_stopped_keeps_the_session_and_leaves_a_failing_status(
    ...
    code, said = _piped(home, _environment(base, home, _first()))

    equal(code, 0, describing=f"the session's own status, which a stop must not end: {said}")
    contains(said, "cannot be verified", describing="the stop's own sentence")
...
def test_help_says_the_two_options_and_installs_nothing(
    ...
    code, said = _script(staged(_first()), home, _first(), "-Help")
...
def test_a_release_tag_that_is_not_one_is_refused_before_anything_is_fetched(
    ...
    code, said = _script(staged(_first()), home, _first(), "-Version", "../latest")
...
def test_a_machine_with_no_tar_is_told_where_windows_keeps_one(
    ...
    """Unpacking needs `tar`, and a machine with none is told so before unpacking.
...
def test_the_systems_own_tar_is_taken_before_one_on_the_path(
    staged: Callable[[str], Path], tmp_path: Path
) -> None:
    """A `tar` ahead of the system's own on PATH is not the one that unpacks.

    Git's, on a runner, reads a drive-lettered path as a host to connect to;
    ...
    ahead.mkdir()
    if sys.platform == "win32":
        (ahead / "tar.cmd").write_text("@echo off\r\nexit /b 1\r\n", encoding="utf-8")
    else:
...
def test_an_artifact_that_is_not_an_archive_is_refused_after_its_digest_matched(
    ...
    base = staged(_first())
    _served_as_the_newest(base, _first(), b"not a gzip archive at all")
...
def test_an_archive_naming_a_place_outside_its_own_directory_is_refused(
    ...
    with tarfile.open(fileobj=raw, mode="w") as archive:
        ...
        info.size = 0
        archive.addfile(info, io.BytesIO(b""))
    _served_as_the_newest(base, _first(), gzip.compress(raw.getvalue(), mtime=0))
...
def test_a_destination_nothing_can_write_is_refused_naming_the_option(
    ...
    code, said = _script(staged(_first()), home, _first(), "-To", str(blocking / "bin"))
...
def test_the_directory_is_on_the_users_own_path_for_the_next_window(
    ...
    Read back through a new process rather than through the run's prose; the
    `users_own_path_kept` fixture takes it off again afterwards, as it does
    after every install here, so the host is left as it was found.
    """
    ...
    contains(
        _users_own_path().split(os.pathsep),
        str(into),
