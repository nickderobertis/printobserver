...
from repo_checks.checks_docs import journeys, reference, schema_document, skill
...
def test_an_agents_md_with_no_journey_inventory_block_is_refused(
    tree: Callable[[], Tree],
) -> None:
    """The block is where the inventory is; without it there is no inventory to hold."""
    copy = tree()
    copy.edit("AGENTS.md", "[//]: # (BEGIN journey-inventory)\n", "")

    refused(journeys(copy.repo), "journey-inventory")


def test_an_inventory_entry_that_says_nothing_is_refused(tree: Callable[[], Tree]) -> None:
    """A name with no sentence beside it is a list rather than an inventory."""
    copy = tree()
    copy.edit(
        "AGENTS.md",
        "[//]: # (BEGIN journey-inventory)\n",
        "[//]: # (BEGIN journey-inventory)\n- `a-journey-nobody-described`\n",
    )

    refused(journeys(copy.repo), "says nothing about it")


def test_a_marker_separated_from_its_test_by_a_blank_line_names_no_journey(
    tree: Callable[[], Tree],
) -> None:
    """The marker names the test on the line below it, so a gap breaks the naming."""
    copy = tree()
    copy.append(SUITE, "\n// journey: a-marker-adrift\n\n#[test]\nfn nothing_at_all() {}\n")

    accepted(journeys(copy.repo), describing="a marker a blank line above a test")


def test_a_marker_at_the_end_of_a_file_names_no_journey(tree: Callable[[], Tree]) -> None:
    """There is nothing below it to be a journey."""
    copy = tree()
    copy.append(SUITE, "\n// journey: a-marker-at-the-end\n")

    accepted(journeys(copy.repo), describing="a marker with nothing below it")
